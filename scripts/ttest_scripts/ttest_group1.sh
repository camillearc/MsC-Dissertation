...
# ttest Group 1: cognitive, first, negative categories and their matches

# Start time
start_time=$(date +"%Y-%m-%d %H:%M:%S")
echo "####################################################################################################"
echo "Starting ttest_group1.sh at $start_time"
...
for perp in "${perps[@]}"; do
    ...
    declare -A word_group_subbricks=(
        ...
        [negative]="133 135 137 139 141 143 145 147 149 151 153 155 157 159 161 163"
    )

    declare -A match_subbricks=(
        [cog_match]="34 36 38 40 42 44 46 48 50 52 54 56 58 60 62 64"
        [first_match]="100 102 104 106 108 110 112 114 116 118 120 122 124 126 128 130"
        [negative_match]="166 168 170 172 174 176 178 180 182 184 186 188 190 192 194 196"
    ...
    declare -A comparisons=(
        [cognitive]=cog_match
        [first]=first_match
        [negative]=negative_match
        [cognitive_vs_others]=others
        [first_vs_others]=others
        [negative_vs_others]=others
    )
